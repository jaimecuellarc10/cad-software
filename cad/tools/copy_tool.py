...
from ..undo import CopyEntitiesCommand

PREVIEW_COLOR = QColor("#ffffff")


class CopyTool(BaseTool):
    """Select entities first, then CO: pick base point → pick destination → copies placed."""
    ...
    def prompt(self) -> str:
        if not self._entities:
            return "COPY  No objects selected.  Esc to cancel."
        if self._base is None:
            return f"COPY  {len(self._entities)} object(s)  Specify base point:"
        return "COPY  Specify destination point  [Esc = cancel]"
    ...
    def _commit(self, dest: QPointF):
        ...
        self.view.undo_stack.push(
            CopyEntitiesCommand(self.view.cad_scene, self._entities, dx, dy)
        )
        # Stay in tool with same entities so user can place more copies
        self._base = None
        if self.view:
            self.view.viewport().update()
